    ...
    public class DangKyXetTuyensController : Controller
    {
        ...
        public ActionResult DkxtkqDgnl(string filteriDotxt, string filteriNvong, string filteriNganh, string filteriLePhi, string filteriHoSo, string currentFilter, string searchString, int? page)
        {
            var model = (from h in db.DangKyXetTuyenKhacs
                                   select h).OrderBy(x => x.ThiSinh_ID).ThenBy(x => x.Dkxt_ToHopXT).ThenBy(x => x.Dkxt_NguyenVong).ThenBy(x => x.Nganh.Nganh_TenNganh).Include(d => d.ThiSinhDangKy.DoiTuong).Include(d => d.DotXetTuyen).Include(d => d.ThiSinhDangKy.KhuVuc).Include(d => d.Nganh).Include(d => d.PhuongThucXetTuyen).Include(d => d.ThiSinhDangKy);
